...
import { Search, Loader2, Plus, Box, Square, Palette, Maximize, Minimize2, Maximize2, Settings2, ChevronUp, ChevronDown, RotateCcw, Focus } from 'lucide-react';
...
import { useGraphStore, type ColorByOption, type SizeByOption } from '@/lib/stores';
...
export function GraphControls({
  ...
  const [customLimit, setCustomLimit] = useState<string>(currentLimit?.toString() || '500');
  const {
    filters,
    setFilters,
    resetFilters,
  ...
  const handleCustomLimitKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
  ...
  return (
    <>
      ...
      <div className={`absolute top-2 left-2 md:top-4 md:left-4 bottom-2 md:bottom-4 hidden md:block ${isFullscreen ? 'z-[60]' : 'z-10'}`}>
        <div className="h-full overflow-y-auto overflow-x-hidden pr-2 flex flex-col gap-2" style={{ scrollbarWidth: 'thin' }}>
          {/* Search */}
          ...
          {entityCount !== undefined && (
            <div className="space-y-2">
              <div className="bg-background/90 border rounded-lg px-3 py-1.5 text-xs text-muted-foreground">
                Showing {entityCount} {currentLimit && `/ ${currentLimit}`} entities
              </div>
              
              {/* Custom entity limit input */}
              <div className="bg-background border rounded-lg p-2 space-y-2">
                <label className="text-xs text-muted-foreground">Load Entities:</label>
          ...
          <div className="bg-background border rounded-lg p-2">
            ...
            >
              ...
              <SelectContent className="z-[100]" container={portalContainer}>
          ...
          <div className="bg-background border rounded-lg p-2">
            <div className="flex items-center gap-2">
              ...
              <ToggleGroup
                ...
                className="gap-1"
              >
                <ToggleGroupItem value="2d" size="sm" className="h-7 px-2 text-xs">
                  <Square className="h-3 w-3 mr-1" />
                  2D
                </ToggleGroupItem>
                <ToggleGroupItem value="3d" size="sm" className="h-7 px-2 text-xs">
      ...
      <div
        className={`${isFullscreen ? 'fixed' : 'absolute'} bottom-0 left-0 right-0 z-[51] md:hidden`}
